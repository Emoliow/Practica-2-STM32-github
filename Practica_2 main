...
volatile bool blink = false; //si es true, enciende, si es false es apagado
/* USER CODE END PV */
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  MX_ADC1_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_Base_Start_IT(&htim6); //Aquí empieza el conteo por interrupcion, y siempre va a entrar al callback del TIMER
  HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_1); // Iniciamos el PWM
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  switch (devState) {
		  ...
		  case PARPADEO:
			  if(blink){
				  HAL_GPIO_WritePin(LED_VERDE_GPIO_Port, LED_VERDE_Pin,1);
				  HAL_GPIO_WritePin(LED_AMARILLO_GPIO_Port,
				  LED_AMARILLO_Pin,1);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if (GPIO_Pin == BTN_Pin){
		if (devState == CONTROL){ //Si ya llegó al ultimo estado, vuelve al estado 0 IDLE.
			devState = IDLE;
		} else {
...
{
	if (htim->Instance == TIM6){ // Timer 500 ms
		if(devState == PARPADEO){
			if(blink){
			    blink = false;
			} else {
			    blink = true;
			}
			htim->Instance->CNT = 0;  // Reinicia el contador a 0
		}
