...
#include "math.h"
#include <string.h>
#define ADC_BUFFER_SIZE 300 //150 cada CHANNEL de ADC
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
volatile States devState = IDLE;
uint32_t LDR1[150] = {0};
uint32_t LDR2[150] = {0};
uint32_t buffer[300] = {0};
float32_t LDR1_V[150] = {0.0f};
float32_t LDR2_V[150] = {0.0f};
bool button_flag = true;

//Punteros
uint32_t* ptr1 = &buffer[0];
uint32_t* ptr2 = &buffer[1];
uint32_t* ptrLDR1 = &LDR1[0];
uint32_t* ptrLDR2 = &LDR2[0];


uint16_t i=0;
float32_t valorVolt= 0;
uint32_t valorDigital=0;
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  float32_t mean_ldr1;
  float32_t min_ldr1;
  uint32_t index_min_ldr1;
  float32_t max_ldr1;
  uint32_t index_max_ldr1;
  float32_t std_ldr1;
  float32_t rms_ldr1;
  ...
  float32_t mean_ldr2;
  float32_t min_ldr2;
  uint32_t index_min_ldr2;
  float32_t max_ldr2;
  uint32_t index_max_ldr2;
  float32_t std_ldr2;
  float32_t rms_ldr2;
  bool cont = true;
  bool cont2 = true;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if (GPIO_Pin == USER_BTN_Pin){
		if(button_flag == true){
			if (devState == PROCESS){ //Si ya llegó al ultimo estado, vuelve al estado 0 IDLE.
				devState = IDLE;
			} else {
...
void valoresVoltajeLDR1(){
	uint32_t* ptrLDR1 = &LDR1[0];
	for(i=0;i<150;i++) {
		valorDigital= *ptrLDR1;
		valorVolt= (valorDigital*ADC_VREF)/pow(2,16);
		LDR1_V[i]=valorVolt;
		ptrLDR1++;
	}
}

void valoresVoltajeLDR2(){
	uint32_t* ptrLDR2 = &LDR2[0];
	for(i=0;i<150;i++) {
		valorDigital= *ptrLDR2;
		valorVolt= (valorDigital*ADC_VREF)/pow(2,16);
		LDR2_V[i]=valorVolt;
		ptrLDR2++;
	}
