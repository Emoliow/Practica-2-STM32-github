...
typedef struct{
	...
	volatile float temperature; //20 bits
	volatile float pressure; //20 bits
	volatile float humidity; //16 bits
...
volatile uint8_t tempBuffer[6];
volatile uint8_t humBuffer[8];
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  if (myBool == true){
	  //Config BME En Blocking:
		  //Registro 0xF2 (Ctrl hum):
	  uint8_t ctrlHum = 0x01;
	  HAL_I2C_Mem_Write(&hi2c2, BME280_I2C_ADDRESS, BME280_REG_CTRL_HUM, 1, &ctrlHum, 1, 1000);

		  //Registro 0xF4 (Ctrl)
	  uint8_t ctrlMeas = 0b00100101;
	  HAL_I2C_Mem_Write(&hi2c2, BME280_I2C_ADDRESS, BME280_REG_CTRL_MEAS, 1, &ctrlMeas, 1, 1000);

		  //Registro 0xF5 (Config)
	  uint8_t config = 0;
	  HAL_I2C_Mem_Write(&hi2c2, BME280_I2C_ADDRESS, BME280_REG_CONFIG, 1, &config, 1, 1000);


	  // Leer los calibration data en blocking

	  HAL_I2C_Mem_Read(&hi2c2, BME280_I2C_ADDRESS, BME280_REG_T1, 1, &tempBuffer[0], 6, 1000);
	  HAL_I2C_Mem_Read(&hi2c2, BME280_I2C_ADDRESS, BME280_REG_H1, 1, &humBuffer[0],1, 1000);
	  HAL_I2C_Mem_Read(&hi2c2, BME280_I2C_ADDRESS, BME280_REG_H2, 1, &humBuffer[1], 7, 1000);

	  devBME280.coeficientes.dig_T1 = tempBuffer[0] | (tempBuffer[1] <<8);
	  devBME280.coeficientes.dig_T2 = tempBuffer[2] | (tempBuffer[3] <<8);
	  devBME280.coeficientes.dig_T3 = tempBuffer[4] | (tempBuffer[5] <<8);

	  devBME280.coeficientes.dig_H1 = humBuffer[0];
	  devBME280.coeficientes.dig_H2 = humBuffer[1] | (humBuffer[2]<<8);
	  devBME280.coeficientes.dig_H3 = humBuffer[3];
	  devBME280.coeficientes.dig_H4 = (humBuffer[5] & 0x0F) | (humBuffer[4]<<4);
	  devBME280.coeficientes.dig_H5 = (humBuffer[5] & 0xF0)>>4 | (humBuffer[6]<<4);
	  devBME280.coeficientes.dig_H6 = humBuffer[7];
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if(GPIO_Pin == USER_BTN_Pin){
		if(devState == MODO_HUM){ //Si ya llegó al último estado, vuelve al estado 0 IDLE
			devState = IDLE;
		} else{
			devState++; //Aqui va cambiando el estado de manera secuencial
		}
	}
}
